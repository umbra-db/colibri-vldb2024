#include "umbra/hattrick/HattrickRefreshClient.hpp"
...
HattrickRefreshClient::HattrickRefreshClient(const Database& database, unsigned clientId)
   : UmbraClient(database), transactionGenerator(42 + clientId), generator(42 + clientId)
// Constructor
{
}
//---------------------------------------------------------------------------
...
// Initialize the client
{
   ...
   auto count = [this](const string& table) {
      auto result = connection.execute("SELECT COUNT(*) FROM " + table);
