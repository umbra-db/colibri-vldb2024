...
HyBenchOLTPClient::HyBenchOLTPClient(const Database& database, HyBenchSharedState& state, unsigned clientId)
   : UmbraClient(database), config(hybench::Config::instantiate(database.getScaleFactor())), generator(config, clientId), transactionGenerator(42 + clientId, true), state(state)
// Constructor
{
}
//---------------------------------------------------------------------------
