...
#include "umbra/hybench/HyBenchOLTPClient.hpp"
#include "umbra/hybench/HyBenchQueryClient.hpp"
...
HyBenchDriver::HyBenchDriver(Database& database, unique_ptr<DriverConfig> driverConfig, string logfile)
   : UmbraDriver(database, std::move(driverConfig), std::move(logfile)), mode(OLAP), originalLogfile(this->logfile)
// Constructor
{
...
// Run the driver
{
   ...
   if (database.getAnalyticalClients() != 0) {
      ...
      clients.clear();
      if (!UmbraDriver::run(true)) return false;
   }
