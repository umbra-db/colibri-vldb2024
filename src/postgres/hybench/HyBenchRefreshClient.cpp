...
HyBenchRefreshClient::HyBenchRefreshClient(const Database& database, HyBenchSharedState& state, unsigned clientId)
   : PostgresClient(database), config(hybench::Config::instantiate(database.getScaleFactor())), generator(config, clientId), transactionGenerator(42 + clientId, false), state(state)
// Constructor
{
}
//---------------------------------------------------------------------------
