...
TpchRefreshClient::TpchRefreshClient(const Database& database, const odbc::Environment& environment)
   : TiDBClient(database, environment)
// Constructor
{
}
//---------------------------------------------------------------------------
...
// Perform work
{
   ...
   while (running()) {
      ...
      connection.exec("LOAD DATA LOCAL INFILE '/data/update/orders.tbl.u" + to_string(index + 1) + "' INTO TABLE orders FIELDS TERMINATED BY '|'");
      connection.exec("LOAD DATA LOCAL INFILE '/data/update/lineitem.tbl.u" + to_string(index + 1) + "' INTO TABLE lineitem FIELDS TERMINATED BY '|'");
      connection.exec("COMMIT");

      // Remove old sales
      connection.exec("BEGIN");
      connection.exec("CREATE TEMPORARY TABLE to_delete (id INTEGER NOT NULL)");
      connection.exec("LOAD DATA LOCAL INFILE '/data/delete/delete." + to_string(index + 1) + "' INTO TABLE to_delete FIELDS TERMINATED BY '|'");
